...
function init() {
  for (let i = 0; i < slides.length; i++) {
    ...
    slide.style.transform = "translate(-35%, -35%)";
...
function update() {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    if (leftSlide) {
      ...
      leftSlide.style.left = `${width / 2 + a * Math.cos((Math.PI * 3) / 2 - delta * i * 2.7)}px`;
    ...
    if (rightSlide) {
      ...
      rightSlide.style.left = `${width / 2 + a * Math.cos((Math.PI * 3) / 2 + delta * i * 2.7)}px`;
